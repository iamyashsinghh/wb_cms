...
<script>
    ...
    function update_status(elem){
        if(confirm("Are you sure want to update the status")){
            ...
            fetch(`{{route('vendor.listing_meta.update_status')}}/${data_id}/${data_status}`).then(response => response.json()).then(data => {
    ...
    function update_faq(faq_id){
        fetch(`{{route('vendor.listing_meta.fetch_faq')}}/${faq_id}`).then(response => response.json()).then(data => {
            ...
            updateFaqModal.querySelector('form').action = `{{route('vendor.listing_meta.update_faq')}}/${faq_id}`;
