@extends('layouts.app')
@section('title', "Venue Listing Meta")
@section('header-css')
...
<div class="content-wrapper pb-5">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Venue Listing Meta</h1>
                </div>
            </div>
            @canany(['create venue_vendor_list', 'super power'])
            <div class="button-group my-4">
                <a href="{{route('venue.listing_meta.manage')}}" class="btn btn-sm text-light buttons-print" style="background-color: var(--wb-renosand)"><i class="fa fa-plus mr-1"></i>Add New</a>
            </div>
            @endcanany
        </div>
    </section>
...
<script>
    $('.summernote').summernote({
     ...
     $(document).ready(function() {
        $('#serverTable').DataTable({
            ...
            serverSide: true,
            ajax: `{{route('venue.listing_meta.ajax_list')}}`,
            order: [
                [0, 'desc']
            ],
            rowCallback: function(row, data, index) {
                ...
                <a href="{{route('venue.listing_meta.manage')}}/${data[0]}" class="text-success mx-2" title="Edit">
    ...
    function update_status(elem){
        if(confirm("Are you sure want to update the status")){
            ...
            fetch(`{{route('venue.listing_meta.update_status')}}/${data_id}/${data_status}`).then(response => response.json()).then(data => {
    ...
    function update_faq(faq_id){
        fetch(`{{route('venue.listing_meta.fetch_faq')}}/${faq_id}`).then(response => response.json()).then(data => {
            console.log(data)
            ...
            updateFaqModal.querySelector('form').action = `{{route('venue.listing_meta.update_faq')}}/${faq_id}`;
