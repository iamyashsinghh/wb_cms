                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="badge text-xs text-light" style="background: var(--wb-dark-red);">Venue</span>
                                @else
                                <span class="badge text-xs text-light" style="background: var(--wb-renosand);">Vendor</span>
                                @endif
                            </td>
                            <td>{{$list->business_type == 1 ? $list->get_venue_category->name : $list->get_vendor_category->name}}</td>
                            <td>{{$list->phone}}</td>
                            <td>{{$list->email}}</td>
                            ...
                            <td>
                                ...
                                @endphp
                                <a href="{{$link}}" style="font-size: 22px;"><i class="fa {{$element_class}}" onclick="return confirm('Are you sure want to update the status?')"></i></a>
                            </td>
                            <td>
                                ...
                                @else
                                <div class="dropdown">
                                    <button class="btn btn-xs rounded-pill btn-{{$list->content_status == 1 ? 'warning' : 'danger'}} dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">{{$list->content_status == 1 ? 'Pending': 'Reject'}}</button>
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="{{route('business_user.manage_content', $list->id)}}" target="_blank">Update</a></li>
                                        <li><a class="dropdown-item" href="{{route('business_user.update_content_status', [$list->id, 2])}}">Reject</a></li>
                                    </ul>
                                </div>
                                @endif
                            </td>
                            <td>
                                ...
                                <div class="dropdown">
                                    <button class="btn btn-xs rounded-pill btn-{{$list->content_status == 1 ? 'warning' : 'danger'}} dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">{{$list->content_status == 1 ? 'Pending': 'Reject'}}</button>
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="{{route('business_user.manage_images', $list->id)}}">Update</a></li>
                                        <li><a class="dropdown-item" href="{{route('business_user.update_images_status', $list->id, 2)}}">Reject</a></li>
                                    </ul>
                                </div>
                                @endif
                            </td>
                            <td class="text-center">
                                ...
                                <a href="javascript:void(0);" data-user-id="{{$list->id}}" data-bussiness_type-id="{{$list->business_type}}" onclick="handle_user_migrate(this)" class="btn p-0"><span class="badge text-xs badge-warning">Click to migrate</span></a>
                                @else    
                                {{$list->migrated_business_id}}
                                @endif
                            </td>
                            <td class="text-center text-nowrap">
                                <a href="javascript:void(0);" onclick="handle_business_user_edit({{$list->id}})" class="text-success mx-2" title="Edit">
                                    <i class="fa fa-edit" style="font-size: 15px;"></i>
                                </a>
                                <a href="{{route('business_user.delete', $list->id)}}" onclick="return confirm('Are you sure want to delete?')" class="text-danger mx-2" title="Delete">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4 mb-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" placeholder="Enter phone no." name="phone_number" required minlength="11" maxlength="11">
                                </div>
                            </div>
                            <div class="col-sm-4 mb-3">
                                <div class="form-group">
    ...
    function handle_business_user_edit(user_id){
        ...
        fetch(`{{route('business_user.edit')}}/${user_id}`).then(response => response.json()).then(data => {
            if(data.success == true){
                ...
                const business_category = editBusinessUserModal.querySelector('select[name="business_category"]');
                business_category.innerHTML = "";
                for(let category of data.categories){
                    business_category.innerHTML += `<option value="${category.id}" ${data.user.business_category_id == category.id ? 'selected': ''}>${category.name}</option>`;
                }
