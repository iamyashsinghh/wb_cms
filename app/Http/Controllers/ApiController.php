...
//         // Fetch all vendors matching the criteria before pagination
//         $all_vendors = $data->get();

//         // Apply services and occasions filtering
//         if ($tag == 'vendors') {
//             $all_vendors = $all_vendors->map(function ($vendor) {
//                 $services = json_decode($vendor->services, true);
//                 $occasions = json_decode($vendor->occasions, true);
...
//                 $all_vendors = $all_vendors->filter(function ($vendor) use ($photographerService, $photographerOccasion, $makeupService, $makeupOccasion) {
...
//         $total_items = $all_vendors->count();

//         // Apply pagination to the filtered vendors
//         if ($location_slug != 'all') {
//             $venues_or_vendors = $all_vendors->sortBy('locationid')->sortByDesc('popular')->sortByDesc('id')->slice($offset, $items_per_page);
//         } else {
//             $venues_or_vendors = $all_vendors->sortByDesc('popular')->sortByDesc('id')->slice($offset, $items_per_page);
//         }
...
//             'tag' => $tag,
//             'count' => $total_items,
//             'data' => $venues_or_vendors->values()->all(), // Reset the keys of the collection
//             'meta' => $meta,
//             'cities' => $cities,
//             'pagination' => [
//                 'current_page' => $page_no,
