    ...
    public function venue_or_vendor_list(Request $request, string $category_slug, string $city_slug, string $location_slug = 'all', int $page_no = 1)
{
    try {
        ...
        if ($venue_category) {
            ...
            $meta = Cache::remember("meta_$slug", 60, function() use ($slug) {
                return VenueListingMeta::select('meta_title', 'meta_description', 'meta_keywords', 'caption', 'faq')
                    ->where('slug', $slug)->first();
            });
            ...
            $meta = Cache::remember("meta_$slug", 60, function() use ($slug) {
                return VendorListingMeta::select('meta_title', 'meta_description', 'meta_keywords', 'caption', 'faq')
                    ->where('slug', $slug)->first();
            });
