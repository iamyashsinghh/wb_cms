...
class ApiController extends Controller
{
    ...
    //                 if (!in_array($newValue, $localityIds)) {
    //                     $localityIds[] = $newValue;
    //                 }
    ...
    //         $venues_or_vendors = $data->orderBy('popular', 'desc')->offset($offset)->limit(9)->get();
    ...
    //         $venues_or_vendors = $data->orderBy('popular', 'desc')->offset($offset)->limit($items_per_page)->get();
    ...
    public function venue_or_vendor_details(string $slug)
    {
        try {
            ...
            $is_redirect = 0;
            if ($venue) {
                $similar_packages = Venue::select('id', 'name', 'images', 'venue_address', 'phone', 'slug', 'min_capacity', 'max_capacity', 'veg_price', 'nonveg_price', 'wb_assured', 'is_redirect')->whereIn('id', explode(',', $venue->similar_venue_ids))->get();
                $data['venue'] = $venue;
                ...
                $similar_vendors = Vendor::select('id', 'brand_name', 'package_price', 'vendor_address', 'phone', 'slug', 'images', 'wb_assured', 'is_redirect')->whereIn('id', explode(',', trim($vendor->similar_vendor_ids)))->get();
                $data['vendor'] = $vendor;
