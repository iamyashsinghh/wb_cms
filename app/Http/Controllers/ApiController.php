    ...
    public function venue_or_vendor_list(Request $request, string $category_slug, string $city_slug, string $location_slug = 'all', int $page_no = 1)
{
    try {
        ...
        // Modify the ordering based on locality->id and location->locality_ids
        $data->orderBy('locations.id', 'asc')
             ->orderBy('locations.locality_ids', 'asc')
             ->orderBy('popular', 'desc')
             ->orderBy('venues.id', 'desc');

        $total_items = $data->count();
        $venues_or_vendors = $data->skip($offset)->take($items_per_page)->get();
